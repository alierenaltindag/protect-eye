...
echo "==> Fedora / Red Hat / openSUSE (.rpm) paketi oluşturuluyor..."
...
if [ -n "$RPM_FILE" ]; then
    echo "==> Fedora .rpm paketi başarıyla üretildi: $RPM_FILE"
    cp -f "$RPM_FILE" "$SCRIPT_DIR/"
    ls -lh "$SCRIPT_DIR/"*.rpm
else
    echo "==> Hata: .rpm paketi üretilemedi. 'rpm-build' (veya 'rpmbuild') aracının kurulu olduğundan emin olun."
    exit 1
fi
